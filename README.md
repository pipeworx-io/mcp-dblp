...
`tools/list` at `https://gateway.pipeworx.io/dblp/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/dblp_search_publications \
  -H 'Content-Type: application/json' \
  -d '{"query":"deep learning neural networks"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/dblp_search_publications`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
